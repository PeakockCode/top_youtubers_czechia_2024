...
SELECT CAST(LEFT(NAME, CHARINDEX('@', NAME)-1) AS NVARCHAR(100)) AS Name, total_subscribers AS Total_Subscribers, total_views AS Total_Views, total_videos AS Total_Videos
FROM social_media_db..TopCzechYoutubers2024;
-- second example
...
1.	The data needs 100 rows of records of Youtube channels - row count check
	EXPECTATION: 100
2.	The data needs 4 fields (Channel_Name, Total_Subscribers, Total_Views and Total_Videos) - column count and name check
	EXPECTATION: 4
...
---------------------------------------------------------------------------------------------------------
-- 2A column count check - 4 fields - OK (PASSED!)
...
-- 2B column names check: Channel_Name, Total_Subscribers, Total_Views and Total_Videos - All names OK (PASSED!)
...
WHERE 
	TABLE_NAME = 'ViewTopCzechYoutubers2024';
---------------------------------------------------------------------------------------------------------
-- 3 data type check - 4 fields - OK, Channel_Name = strings and all the others are numeric, (PASSED!)
